...
import { useListTraceabilityLots, useGetTraceabilityLot } from "@workspace/api-client-react";
import { Card, CardContent } from "@/components/ui/card";
import { Input } from "@/components/ui/input";
import { Button } from "@/components/ui/button";
import { Search, Map, ChevronRight, Package, Truck, Beaker, CheckCircle2, Factory } from "lucide-react";
...
export default function Traceability() {
  ...
  return (
    <div className="flex-1 flex flex-col">
      <PageHeader 
        title="Traceability" 
        description="Track product lifecycle from grove to glass."
      />
      ...
      <div className="p-6 sm:p-8 flex-1 overflow-y-auto space-y-6">
          ...
          <CardContent className="p-6">
            <form onSubmit={handleSearch} className="flex gap-4">
              <div className="relative flex-1">
                ...
                <Input 
                  type="search" 
                  placeholder="Enter Lot Code (e.g. LOT-12345)..." 
        ...
        {isError && (
          <div className="max-w-3xl mx-auto mt-12 py-12 flex flex-col items-center justify-center text-muted-foreground text-sm border border-dashed rounded-lg bg-muted/30">
            ...
            <p className="text-lg font-medium text-foreground mb-1">Lot not found</p>
            <p>Could not find any traceability records for "{searchedLot}".</p>
          </div>
        )}
          ...
          <div className="max-w-4xl mx-auto mt-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <Card className="shadow-sm border-border/50 mb-8 bg-primary/5 border-primary/20">
              <CardContent className="p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                  <p className="text-sm font-medium text-primary mb-1">Traceability Report</p>
                ...
                <div key={event.id} className="relative pl-8">
                  ...
                  <Card className="shadow-sm border-border/50">
                    <CardContent className="p-5">
                      ...
                      {event.actor && (
                        <p className="text-sm font-medium text-foreground mb-3">By: {event.actor}</p>
                      )}
